<!-- resources/views/teacher/material-edit.blade.php -->
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <main>
        <div class="container">
            <div class="form-container">
                ...
                <form action="{{ route('teacher.material.update', [$class->id, $material->id]) }}" method="POST" class="material-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select id="category" name="category" required>
                                ...
                                <option value="scratch" {{ old('category', $material->category) == 'scratch' ? 'selected' : '' }}>🎮 Scratch</option>
                                <option value="picto-ai" {{ old('category', $material->category) == 'picto-ai' ? 'selected' : '' }}>🤖 Picto AI</option>
                            </select>
                            @error('category')
                                <div class="error">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <select id="difficulty" name="difficulty" required>
                                ...
                                <option value="beginner" {{ old('difficulty', $material->difficulty) == 'beginner' ? 'selected' : '' }}>Beginner</option>
                                <option value="intermediate" {{ old('difficulty', $material->difficulty) == 'intermediate' ? 'selected' : '' }}>Intermediate</option>
                                <option value="advanced" {{ old('difficulty', $material->difficulty) == 'advanced' ? 'selected' : '' }}>Advanced</option>
                            </select>
                            @error('difficulty')
                                <div class="error">{{ $message }}</div>
                            @enderror
